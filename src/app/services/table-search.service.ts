...
import {
  ...
  ValidKeywords,
  ValidPlusMinus,
} from '../models/table-search.model';
...
export class TableSearchService {
  ...
  private hasTwoParenthesisFrontAndEnd(search: string) {
    const searchNoFrontAndBack: string = search
      .trim()
      .replace(/^(\()(.*)(\))$/, '$2');
  ...
  private inBetweenHasCorrectNumberQuotes(search: string) {
    const searchNoFrontAndBack: string = search
      .trim()
      .replace(/^(\()(.*)(\))$/, '$2');
    return searchNoFrontAndBack
      .split(',')
      .reduce((accum: boolean, e: string) => {
        accum = accum && e.trim() === "''";
        return accum;
      }, true);
  }
  ...
  private getStatementPossibilities(validKeywords: ValidKeywords[]): string[] {
    return this.validColumnNames.reduce(
      (accum: string[], columnName: string) => {
  ...
  private getRightHandSide(possibilities: string[], sqlCommand: string) {
    ...
    return possibilities
      .reduce((accum: string, possibility: string) => {
  ...
  private isStringAValidDateOrNumber(search: string) {
    const validDate: boolean = /^\d{4}-\d{2}-\d{2}$/.test(
      search.replace(/"/g, '').replace(/'/g, ''),
    );
    const validNumber: boolean = /^\d+$/.test(
      search.replace(/"/g, '').replace(/'/g, ''),
    );
  ...
  public isValidSql(search: string): boolean {
    ...
    if (valid) {
      ...
      search = search.replace(/('.*?')/g, (match, contents) => {
        return "''";
      });
      ...
      for (let i = 0; i < sqlCommands.length; i++) {
        ...
        const equalPossibilities: string[] = this.getStatementPossibilities([
          ValidKeywords.EQUALS,
        ]);
        const inPossibilities: string[] = this.getStatementPossibilities([
          ValidKeywords.IN,
        ]);
        ...
        const betweenPossibilities: string[] = this.getStatementPossibilities([
          ValidKeywords.BETWEEN,
        ]);
        ...
        if (equalPossibilities.find((x) => command.includes(x))) {
          ...
          valid = valid && rightSide === "''";
          const passedInAtIndex: string = passedInSearch
            .substring(1)
            .split('and')[i];
          const rightSidePassedIn = this.getRightHandSide(
            gtLtPossibilities,
            passedInAtIndex,
          );
          valid =
            valid &&
            (this.isStringAValidDateOrNumber(rightSidePassedIn) ||
              [ValidPlusMinus.TODAY, ValidPlusMinus.TODAY_PlUS_MINUS].includes(
                this.validPlusMinus(rightSidePassedIn),
              ));
        } else if (betweenPossibilities.find((x) => command.includes(x))) {
          atLeastOneCommandHit = true;
          const rightSide = this.getRightHandSide(
          ...
          valid =
            valid &&
            this.getSearchArray(rightSidePassedIn).every((e) =>
              this.isStringAValidDateOrNumber(e),
            );
        }
  ...
  private splitIgnoreInParenthesis(
    ...
    for (let i = 0; i < searchStatementIntoCharArr.length; i++) {
      ...
      if (
        numQuotes > 0 &&
        numQuotes % 2 === 0 &&
        skipIndexes.includes(i) &&
        skipIndexes[skipIndexes.length - 1] === i
      ) {
  ...
  private todayMinusPlus(input: string) {
    ...
    if (validPlusMinus === ValidPlusMinus.TODAY) {
      ...
      const addMonthsOrDays = day
        ? moment(new Date()).add(timesSign * Number(unit), 'd')
  ...
  private getSearchArray(searchStatement: string): string[] {
    ...
    return this.splitIgnoreInParenthesis(searchStatement, ',').map(
      (e: string) => {
  ...
  private greaterThan(base: string, compare: string) {
    return (
      Number(base.replace(/[^\d]/g, '')) > Number(compare.replace(/[^\d]/g, ''))
    );
  }

  private greaterThanEqual(base: string, compare: string) {
    return (
      Number(base.replace(/[^\d]/g, '')) >=
      Number(compare.replace(/[^\d]/g, ''))
    );
  }
  ...
  public executeSql(searchStatement: string, dataArray: Data[]): SqlResults {
    ...
    if (valid) {
      ...
      const sqlCommands: string[] = this.splitIgnoreInParenthesis(
        searchStatement,
        'and',
      );
      ...
      sqlCommands.forEach((command: string) => {
        ...
        const comparisonType: ComparisonType = equalPossibilities.find((x) =>
          command.includes(x),
        )
          ? ComparisonType.EQUALS
          : inPossibilities.find((x) => command.includes(x))
            ? ComparisonType.IN
            : gtLtPossibilities.find((x) => command.includes(x))
              ? ComparisonType.LESS_THAN_GREATER_THAN_EQUALS
              : betweenPossibilities.find((x) => command.includes(x))
        ...
        if (comparisonType === ComparisonType.EQUALS) {
          ...
          comparisonType === ComparisonType.LESS_THAN_GREATER_THAN_EQUALS
        ) {
          const values: string[] = Object.values(ValidKeywords)
            .sort((a, b) => {
        ...
        if (numberKeywords > 0) {
          ...
          const indexToSearch = this.validColumnNames.indexOf(
            column.toLowerCase(),
          );
          dataResults = dataResults.filter((data) => {
            if (
              ...
              return (
                this.greaterThanEqual(data[indexToSearch], searchArray[0]) &&
              ...
              if (command.includes(ValidKeywords.GREATER_THAN_EQUALS)) {
                return searchArray.some((e) =>
                  this.greaterThanEqual(data[indexToSearch], e),
                );
              } else if (command.includes(ValidKeywords.LESS_THAN_EQUALS)) {
                return searchArray.some((e) =>
                  this.lessThanEqual(data[indexToSearch], e),
                );
              } else if (command.includes(ValidKeywords.GREATER_THAN)) {
                return searchArray.some((e) =>
                  this.greaterThan(data[indexToSearch], e),
                );
              } else if (command.includes(ValidKeywords.LESS_THAN)) {
                return searchArray.some((e) =>
                  this.lessThan(data[indexToSearch], e),
                );
              } else {
                return false;
              }
