...
import {Account, Restriction, selectAccounts, selectRestrictions} from "../../../ngrx-store/store.reducer";
...
export class NgrxMainComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.store.select(selectRestrictions).subscribe({
  ...
  public removeRestriction(accountId: number){
